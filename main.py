...
class Product:
    ...
    def __init__(self, prod_type, src_cost):
        ...
        if prod_type.lower() in Product.xpt_mult:
            ...
            self.exch_price = (self.src_cost + self.freight) / (1 - Product.exc_pct - Product.tgt_mgn)
            self.exch_fee = self.exch_price * 0.03
            self.exch_cogs = self.src_cost + self.freight + self.exch_fee
            self.contr_price = (self.src_cost + self.freight_contr) / (1 - Product.tgt_mgn)
    ...
    def tell_all(self):
        ...
        print(f"On the Exchange, sell your {self.prod_type} "
                f"at ${round(self.exch_price,2)}. Your sourcing cost was "
                ...
                f"Your gross margin per unit sold will be ${round(self.exch_price - self.exch_cogs,2)} "
